...
function crearUsuario() {
  ...
  $.ajax({
    method: "POST",
    url: rutaInicial + "CreacionUsuarios.php",
    data: {
      accion: "CrearUsuario",
...
function cargarTabla() {
  $.ajax({
    ...
    success: function (result) {
      console.log(result);
      let tablaUsuarios = document.getElementById("TablaPrincipal");
      $(".tBodyUsuarios").remove();
      let numeroTBody = 0;
      totalTbodys=0;
      let tblBody = new Array();
      tblBody[numeroTBody] = document.createElement("tbody");
      tblBody[numeroTBody].setAttribute("class", "tBodyUsuarios");
      tblBody[numeroTBody].setAttribute("id", "tBodyUsuarios:" + numeroTBody);
      tablaUsuarios.appendChild(tblBody[numeroTBody]);
      for (let i in result.Usuarios) {
        if (i % 5 == 0 && i != 0) {
          numeroTBody += 1;
          tblBody[numeroTBody] = document.createElement("tbody");
          tblBody[numeroTBody].setAttribute("class", "tBodyUsuarios");
          tblBody[numeroTBody].setAttribute(
            "id",
            "tBodyUsuarios:" + numeroTBody
          );
          ...
          tblBody[numeroTBody].innerHTML += `<tr>${
            botonEditar +
            botonEliminar +
            usuario +
            nombre +
...
window.addEventListener("load", function () {
  cargarTabla();
  //botones para controlar el paginado
  $("#btnAnterior").click(function () {
    if (tBodyActual <= 0) {
      return;
    } else {
      document.getElementById(`tBodyUsuarios:${tBodyActual}`).style.display =
        "none";
      tBodyActual -= 1;
      document.getElementById(`tBodyUsuarios:${tBodyActual}`).style.display =
        "";
  ...
  $("#btnSiguiente").click(function () {
    if (tBodyActual >= totalTbodys) {
      return;
    } else {
      document.getElementById(`tBodyUsuarios:${tBodyActual}`).style.display =
        "none";
      tBodyActual += 1;
      console.log(tBodyActual);
      document.getElementById(`tBodyUsuarios:${tBodyActual}`).style.display =
        "";
    }
  });
